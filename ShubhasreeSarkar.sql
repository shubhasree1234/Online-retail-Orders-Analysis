...
#-------------------------------------------------------
 ...
 -- as per the following criteria and sort them in descending order of category: a. If the category is 2050, 
 -- increase the price by 2000 b. If the category is 2051, increase the price by 500 c. 
 -- If the category is 2052, increase the price by 600. Hint: Use case statement. no permanent change in table required.
...
else product_price end as New_price_Change
from product
...
select p2.product_class_desc, p1.product_id, p1.product_desc, p1.product_quantity_avail,
case 
when p2.product_class_desc in ('Electronics', 'Computer') then 
case
when p1.product_quantity_avail <= 10 then "Low Stock"
when (p1.product_quantity_avail >=11 and p1.product_quantity_avail <=30) then "In stock"
when p1.product_quantity_avail >= 31 then "Enough stock"
end
when p2.product_class_desc in ('Stationery', 'Clothes') then 
case 
when p1.product_quantity_avail <= 20 then "Low Stock"
when (p1.product_quantity_avail >=21 and p1.product_quantity_avail <=80) then "In stock"
when p1.product_quantity_avail >= 81 then "Enough stock"
end
else 
case 
when p1.product_quantity_avail <= 15 then "Low Stock"
when (p1.product_quantity_avail >=16 and p1.product_quantity_avail <=50) then "In stock"
when p1.product_quantity_avail >= 51 then "Enough stock"
end 
end as Inventory_Status
from product p1
inner join product_class p2
on p1.product_class_code = p2.product_class_code
order by p1.product_class_code , p1.product_quantity_avail;
...
select country, COUNT(city) as No_of_Cities
from address
...
select DISTINCT a.pincode,oc.customer_id, CONCAT(oc.customer_fname, ' ', oc.customer_lname) as full_name ,a.city, oi.order_id,
 p2.product_class_desc, p1.product_desc, (oi.product_quantity * p1.product_price) as sub_total
...
inner join product_class p2
on p1.product_class_code = p2.product_class_code
where CONVERT(a.pincode, nchar) not like "%0%" AND oh.order_status = 'Shipped'
...
-- and which item has been bought along with it maximum no. of times. Display only one record which has the maximum value for 
-- total quantity in this scenario. (USE SUB-QUERY)(1 ROW)[NOTE : ORDER_ITEMS TABLE,PRODUCT TABLE]
-- select * from product;
-- select * from order_items;

select distinct oi.order_id, p1.product_id, p1.product_desc ,SUM(oi.product_quantity) as total_quantity 
from order_items oi inner join product p1
on oi.product_id = p1.product_id
...
select oc.customer_id, CONCAT(oc.customer_fname, ' ', oc.customer_lname) as full_name ,oc.customer_email, 
oi.order_id,p1.product_desc, oi.product_quantity, (oi.product_quantity * p1.product_price) as sub_total
from  online_customer oc 
...
from carton c
where (c.len*c.width*c.height) >= (select SUM(p1.len*p1.width*p1.height*oi.product_quantity) as total_vol 
...
-- select * from shipper;
select oc.customer_id, CONCAT(oc.customer_fname, ' ', oc.customer_lname) as Customer_full_name , oi.order_id, 
...
order by oc.customer_id, Total_order_Quantity;

-- 9. Write a query to display the order_id, customer id and cutomer full name of customers starting with the alphabet "A" 
-- along with (product_quantity) as total quantity of products shipped for order ids > 10030. (5 ROWS) 
-- [NOTE: TABLES TO BE USED - online_customer, order_header, order_items]
select  oi.order_id, oc.customer_id, CONCAT(oc.customer_fname, ' ', oc.customer_lname) as Customer_full_name ,
SUM(oi.product_quantity) as Total_order_Quantity
from  online_customer oc 
inner join order_header oh
on oc.customer_id = oh.customer_id
inner join order_items oi
on oh.order_id = oi.order_id
where oh.order_status = 'Shipped' AND oc.customer_fname LIKE 'A%' AND oi.order_id > 10030
group by oi.order_id
order by oc.customer_id, Total_order_Quantity;
...
select p2.product_class_code, p2.product_class_desc, SUM(oi.product_quantity) as total_quantity, (oi.product_quantity * p1.product_price) as Total
from  online_customer oc 
inner join address a 
on oc.address_id = a.address_id
inner join order_header oh
...
group by p2.product_class_code, p2.product_class_desc
order by total_quantity DESC
limit 1;
